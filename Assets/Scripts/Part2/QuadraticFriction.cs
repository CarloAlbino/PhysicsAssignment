...
public class QuadraticFriction : MonoBehaviour {
    ...
    void CalculateRampStaticFrictionPushForce()
    {
        ...
        Debug.Log(m_netForce.sqrMagnitude);
    }
    ...
    private Vector3 GetRampToRefObjectDisplacement(Transform ramp, Transform refObject)
    {
        ...
        Vector3 rampDirection = ramp.right * ramp.localScale.x * 0.5f;
        rampEdge += rampDirection;

        refObjectEdgePositive = refObject.position;
        Vector3 refObjectDirection = refObject.right * refObject.localScale.x * 0.5f;
        refObjectEdgePositive += refObjectDirection;

        refObjectEdgeNegative = refObject.position;
        refObjectDirection = refObject.right * refObject.localScale.x * 0.5f * -1.0f;
        refObjectEdgeNegative += refObjectDirection;
    ...
    void Start()
    {
        ...
        // Use the displacement from the edge of the ramp to the edge of the platform
        // use the angle of the ramp

        //CalculateQuadraticEquation(30.0f * Mathf.Deg2Rad, Physics.gravity, new Vector3(7.0f, -4.0f, 0.0f));
